...
if (!(Test-Path "C:\Program Files\OpenSSH\bin\ssh.exe")) {
    ...
    (New-Object System.Net.WebClient).DownloadFile($ssh_download_url, $ssh_exe)
    ...
    Start-Process $ssh_exe "/S /port=2222 /privsep=1 /password=D@rj33l1ng" -NoNewWindow -Wait
    
    Write-Output "Deleting $ssh_exe"
    Remove-Item $ssh_exe    
}
